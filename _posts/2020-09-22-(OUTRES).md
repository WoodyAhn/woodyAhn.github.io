...
$h\_{optimal}(d) = \\left({8\\cdot\\Gamma(d/2 + 1)\\over \\pi^{d/2}}\\cdot (d+4)\\cdot (2\\sqrt{\\pi})^{d}\\right)\\cdot n^{-1\\over d+4}$,    where $n = |DB|$

\* $h\_{optimal}(d)$ 값을 이용해 **$\\epsilon(|S|)$** 을 정의합니다

$\\epsilon(|S|) = \\epsilon\\cdot{h\_{optimal}(|S|)\\over h\_{optimal}(2)}$
...
$r(o) = \\prod\_{S\\in RS(o)}score(o,S)$
